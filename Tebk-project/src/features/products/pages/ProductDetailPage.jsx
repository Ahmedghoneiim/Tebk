...
import { ShoppingCart, Heart, Package, Minus, Plus, ArrowLeft } from 'lucide-react'
    ...
    <div className="page-container py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div className="bg-clinical rounded-2xl flex items-center justify-center aspect-square">
          {displayedImage ? (
            <img src={displayedImage} alt={product.name} className="w-full h-full object-cover rounded-2xl" />
          ) : (
        ...
        <div>
            ...
            <div className="mb-6">
              ...
              <div className="flex flex-wrap gap-2">
                {variants.map(v => (
                  <button
                    key={v.id}
                    onClick={() => { setSelectedVariant(prev => prev?.id === v.id ? null : v); setSizeWarning(false) }}
                    className={cn(
          ...
          <div className="flex items-center gap-4 mb-6">
            ...
            <div className="flex items-center border border-border rounded-xl overflow-hidden">
              <button
                ...
                className="px-3 py-2 hover:bg-clinical transition-colors"
              >
          ...
          <div className="mt-6 pt-6 border-t border-border">
            <p className="text-xs text-muted">
              <span className="font-medium text-ink">Stock:</span> {product.stock > 0 ? `${product.stock} units available` : 'Out of stock'}
            </p>
          </div>
        </div>
      </div>
