  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted text-sm mt-1">{orders.length} {t('orders.title').toLowerCase()}</p>
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border">
              {isLoading
                ...
                : orders.length === 0
                  ? (
                    <tr><td colSpan={6}><EmptyState icon={Package} title={t('orders.no_orders_title')} description={t('orders.no_orders_desc')} /></td></tr>
                  )
                  : orders.map(order => (
                    <tr key={order.id} className="hover:bg-clinical transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <Link to={`/orders/${order.id}`} className="text-secondary text-xs hover:underline">{t('orders.details_link')}</Link>
                      </td>
                    </tr>
                  ))
              }
