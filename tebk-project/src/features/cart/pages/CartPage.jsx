  ...
  if (items.length === 0) {
    return (
      <div className="page-container py-8">
        <h1 className="section-title mb-8">{t('cart.page_title')}</h1>
        <EmptyState
          icon={ShoppingCart}
          title={t('cart.empty')}
          description={t('cart.empty_browse_desc')}
  ...
  return (
    <div className="page-container py-8">
      <div className="flex items-center justify-between mb-8">
        <h1 className="section-title">{t('cart.header').replace('{count}', items.length)}</h1>
        <button onClick={clearCart} className="text-sm text-danger hover:underline">{t('cart.clear_all')}</button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="card h-fit">
          <h2 className="font-semibold text-primary mb-4">{t('cart.summary_title')}</h2>
          <div className="space-y-2 text-sm">
            <div className="flex justify-between text-muted">
              <span>{t('cart.subtotal_items').replace('{count}', items.reduce((s, i) => s + i.quantity, 0))}</span>
              <span>{formatCurrency(subtotal)}</span>
            </div>
            <div className="flex justify-between text-muted">
              <span>{t('cart.shipping')}</span>
              <span>{shipping === 0 ? <span className="text-success">{t('common.free_shipping')}</span> : formatCurrency(shipping)}</span>
            </div>
            {shipping > 0 && (
              <p className="text-xs text-muted">{t('cart.free_shipping_notice').replace('{amount}', formatCurrency(500 - subtotal))}</p>
            )}
          </div>
