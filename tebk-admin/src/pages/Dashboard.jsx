...
export function Dashboard() {
  ...
  const stats = useMemo(() => {
    ...
    const pendingB2BDocs = users.filter(u => (u.role === 'clinic_owner' || u.role === 'supplier') && !u.verified).length
  ...
  const pendingVerif = useMemo(() =>
    users
      .filter(u => (u.role === 'clinic_owner' || u.role === 'supplier') && !u.verified)
      .slice(0, 5)
      .map(u => ({
        id:   u.id,
        name: u.clinic_name || u.full_name || '—',
        doc:  u.role === 'clinic_owner' ? 'license_v2.pdf' : 'tax_card.pdf',
        role: u.role,
      }))
  , [users])
