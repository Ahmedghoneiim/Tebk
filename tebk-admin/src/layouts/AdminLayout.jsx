...
import { TebkIcon } from '@/components/TebkLogo'
    ...
    return (
      <NavLink
        ...
        className={({ isActive }) => cn(
          ...
          isActive
            ? 'bg-white/8 text-white shadow-sm -translate-y-0.5'
            : 'text-white/55 hover:bg-white/8 hover:text-white/90 hover:-translate-y-0.5'
        )}
      >
        {({ isActive }) => (
          <>
            {/* Icon pill — solid indigo when active */}
            <span className={cn(
            ...
            <span className={cn('flex-1', isActive && 'font-semibold')}>{group.label}</span>
            {isActive && <ChevronRight className="w-3.5 h-3.5 text-primary/70 shrink-0" />}
          </>
        )}
      </NavLink>
    )
    ...
    <div>
      <button
        ...
        className={cn(
          ...
          isGroupActive
            ? 'text-white bg-white/5 -translate-y-0.5'
            : 'text-white/55 hover:bg-white/8 hover:text-white/90 hover:-translate-y-0.5'
        )}
      >
        {/* Icon pill — subtle indigo tint when group is active */}
        <span className={cn(
          'w-7 h-7 rounded-lg flex items-center justify-center shrink-0 transition-all duration-150',
          isGroupActive ? 'bg-primary/25 text-primary' : 'text-white/40'
        )}>
          <Icon className="w-4 h-4" />
        </span>
        <span className={cn('flex-1 text-left', isGroupActive && 'font-semibold')}>{group.label}</span>
      ...
      {isExpanded && (
        <div className={cn(
          'mt-0.5 ml-4 pl-3 border-l space-y-0.5 transition-colors duration-150',
          isGroupActive ? 'border-primary/40' : 'border-white/10'
        )}>
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center gap-3 px-5 py-5 border-b border-white/10 shrink-0">
          <TebkIcon className="w-9 h-9 shrink-0" />
